...
const GroupDetail = () => {
  ...
  // Action: leadership schedules a meeting (called from GroupCalendar)
  ...
  // Action: leadership posts an announcement
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/groups/${id}/announcements`,
        {
          ...
          body: JSON.stringify({ text_content: announcementText }),
        },
      );
      if (response.ok) {
